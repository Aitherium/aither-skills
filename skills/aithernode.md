# aithernode — turn a machine's hardware into something your agents can use

**AitherNode is the body of AitherOS.** It's a local [MCP](https://modelcontextprotocol.io) server
...
*act* on that hardware instead of just talking about it. If aither-adk is the mind, AitherNode is the
hands.

## Set it up

With [aither-adk](aither-adk.md) installed (`pip install aither-adk`):
...
An AitherNode is a machine made useful; [AitherConnect](aitherconnect.md) wires it to the control
plane and mesh so agents elsewhere can reach it, [aither-adk](aither-adk.md) is the runtime that
