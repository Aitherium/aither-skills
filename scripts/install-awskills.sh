#!/usr/bin/env bash
# install-awskills.sh — install the awskills pack into whatever agents you have.
#
...
#
# Usage:
#   bash scripts/install-awskills.sh [--dry-run] [--force] [--list]
...
if [ "$targets_hit" = 0 ]; then
  ...
  printf '    bash scripts/install-awskills.sh --target openclaw\n'
  printf '    bash scripts/install-awskills.sh --list\n\n'
  exit 1
fi
